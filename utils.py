...
def calc_material_cost(base_price, delivery_km, loads):
    if loads == 0:
        return 0
    base_with_tax = base_price * 1.15
    km_extra = max(0, delivery_km - 30)
    extra_km_cost = km_extra * 4.20 * 1.15
    return loads * (base_with_tax + extra_km_cost)
...
for name, base_price in materials.items():
    ...
    with col1:
        loads = st.number_input(f"Number of Loads for {name}", min_value=0, step=1, key=f"{name}_loads")
    with col2:
        km = st.number_input(f"Delivery Distance for {name} (km)", min_value=0, step=1, key=f"{name}_km")
    cost = calc_material_cost(base_price, km, loads)
